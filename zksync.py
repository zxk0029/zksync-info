...
async def pd_show(args):
    ...
    if args.save:
        def save_df_to_excel_with_color(df, output_file):
            # Initialize Excel writer
            with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
                # Define cell formats for red and green text
                red_format = writer.book.add_format({'color': 'red', 'bold': True})
                green_format = writer.book.add_format({'color': 'green', 'bold': True})
                ...
                # Iterate through cells to apply formats
                for idx, col in enumerate(df.columns):
                    for i, value in enumerate(df[col]):
                        if isinstance(value, str):
                            if '[red]' in value:
                                value = value.replace('[red]', '').replace('[/red]', '')
                                try:
                                    value = float(value)
                                except:
                                    pass
                                worksheet.write(i + 1, idx, value, red_format)
                            elif '[green]' in value:
                                value = value.replace('[green]', '').replace('[/green]', '')
                                try:
                                    value = float(value)
                                except:
                                    pass
                                worksheet.write(i + 1, idx, value, green_format)

        save_df_to_excel_with_color(df, "zks-info.xlsx")
